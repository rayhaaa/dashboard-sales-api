...
async function getTotalTargetSales() {
  try {
    let now = getFormatDate(new Date());
    let start = getFormatDate(new Date(), true);
    let queries = `SELECT
    ...
    WHERE 
        ...
        AND c.PriceList = 1
        AND g.Validfor = 'Y'
    GROUP BY DATEPART(yy, b.Date), DATENAME(MONTH, b.Date), d.CardName;
    `;
    await sql.connect(config);
    let result = await sql.query(queries);
...
module.exports = { getAktualSales, getTargetSales, getTotalTargetSales };
