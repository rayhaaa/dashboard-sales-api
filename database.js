...
async function getEachCustomer(eachCustomer) {
    // const item = objs.slice().reverse().find(obj => obj.action === 'Boot') //untuk nyari id, tapi belum jadi
...
//     // pinjam, ini query detail tiap customer
//     try {
//       await sql.connect(config);
//       let now = getFormatDate(new Date());
//       let start = getFormatDate(new Date(), true);
...
// buat id unik untuk tiap objek, tapi masih gagal
// async function addUniqueId(data) {
//   data.forEach((item, index) => {
//     item.id = index + 1; // You can use any unique ID generation logic here
//   });
// }

// addUniqueId(getTargetSales);

// // Display the modified JSON data
// console.log(getTargetSales);
