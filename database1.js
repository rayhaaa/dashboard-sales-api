...
async function getDataCam() {
  try {
    ...
    const query = `/* SELECT * FROM OFPR T0 */
        ...
        Declare @StartDate Date Declare @EndDate Date declare @GroupMsn nvarchar(40) if isnull(@GroupMsn, '') = ''
        SET
            @GroupMsn = 'All'
        SELECT
            TOP 1 avg(x.CmpltQty / x.PlannedQty) * 100 'percen'
        FROM
            OWOR T0
            ...
            cross JOIN (
                Select
                    SUM(A.CmpltQty) CmpltQty,
                    sum(A.PlannedQty) PlannedQty
                from
                    OWOR A
                    LEFT JOIN [@MASTERMESIN] Y on A.U_MIS_NoMesin = Y.Code
                where
                    A.[UserSign] = 19
                    and A.[Warehouse] = 'WHWIPMF1'
                    and Y.U_MIS_CodeGroup = 'CAM'
